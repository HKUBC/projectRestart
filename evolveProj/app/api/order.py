...
def add_item_to_order( orderId: int, itemId: int, quantity: int, db: Session = Depends(get_db)):
    #get order ID
    ...
    result = db.execute(order_create_query)
    db.commit()
...
def add_items_to_order(orderId: int, order: list[itemsOrderd], db: Session = Depends(get_db)):
    #Get order ID
    find_orderID_query = orders.select().where(orders.c.id == orderId)
...
def get_order_by_userID(user_id: int, db: Session = Depends(get_db)):
...
def get_order_by_orderID(order_id: int, db: Session = Depends(get_db)):
    #
