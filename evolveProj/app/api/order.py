...
def create_order(user_id: Annotated[int, Depends(get_current_user)], db: Annotated[Session, Depends(get_db)]):
    order_id= RAND_ID
...
def add_item_to_order( orderId: int, itemId: int, quantity: int, authenticated_user:  Annotated[int, Depends(get_current_user)], db: Annotated[Session, Depends(get_db)]):
...
@router.post("/add-items/{orderId}", status_code=status.HTTP_201_CREATED)
def add_items_to_order(orderId: int, order: list[itemsOrderd],authenticated_user:  Annotated[int, Depends(get_current_user)], db: Annotated[Session, Depends(get_db)]):
    #Verify that the order exists and belongs to the user 
    order_find_query = orders.select(orders.c.user_id).where(orders.c.id == orderId)
    owner_id = db.execute(order_find_query).scalar()
    if not owner_id:
    ...
    #Get order ID
    find_orderID_query = orders.select(orders.c.id).where(orders.c.id == orderId)
    order_id  = db.execute(find_orderID_query).scalar()
    if order_id is None:
    ...
    db.execute(order_items.insert(), values)
    db.commit()
    return {"message": "Items successfully added to order!"}

@router.get("/user-id/{user_id}", response_model= list[orderItems], status_code=status.HTTP_302_FOUND)
def get_order_by_userID(user_id: Annotated[int, Depends(get_current_user)], db: Annotated[Session, Depends(get_db)]):
